...
class GameScene extends Phaser.Scene {
  ...
  createFastDuck() {
    //generate a random location for a duck to be spawned
    const fastDuckXLocation = Math.floor(Math.random() * 1920) + 1
    //define the duck
    const aFastDuck = this.physics.add.sprite(fastDuckXLocation, -100, 'fastDuck').setScale(3.0)
    //make the duck move
    aFastDuck.body.velocity.y = 1000
    this.fastDuckGroup.add(aFastDuck)
  }
  constructor () {
    super({ key: 'gameScene' })
 ...
 this.cameras.main.setBackgroundColor('#ffffff')
  ...
  preload () {
    ...
    //load the image
    this.load.image('starBackground', 'assets/starBackground.gif')
    //load the spaceship
    ...
    //load the fast duck
    this.load.image('fastDuck', 'assets/fastDuck.png')
    ...
    this.load.audio('explosion', 'audio/barrelExploding.wav')
    this.load.audio('bomb', 'audio/bomb.wav')
    this.load.audio('quack', 'audio/quack.mp3')
  }
  ...
  //give the spaceship physics and determine its starting position
    ...
    //create a group for the fast duck
    this.fastDuckGroup = this.add.group()
    this.createFastDuck()
...
//collisions between the ship and ducks
    ...
    //game over screen
    this.physics.add.collider(this.ship, this.fastDuckGroup, function (shipCollide, fastDuckCollide) {
      //stop the music
      this.game.sound.stopAll()
      //play the sound
      this.sound.play('bomb')
      ...
      this.physics.pause()
      //delete the duck and ship
      fastDuckCollide.destroy()
      shipCollide.destroy()
      //text when the game ends
      this.gameOverText = this.add.text(1920 / 2, 1080 / 2, 'The fast ducks are too cool to get shot\nClick to restart.', this.gameOverTextStyle).setOrigin(0.5)
      //find out if the user quits the game or not
      this.gameOverText.setInteractive({ useHandCursor: true })
      //return to the game scene if the player continues
      this.gameOverText.on('pointerdown', () => this.scene.start('gameScene'))
  ...
  update (time, delta){
    ...
    //make the Duck warp to the top of the screen when it reaches the bottom
    this.duckGroup.children.each(function (item) {
      if (item.y > 1080){
        ...
        //make the Fast Duck warp to the top of the screen when it reaches the back
    this.fastDuckGroup.children.each(function (item) {
      if (item.y > 1080){
        item.y = 0
        const fastDuckXCoordinate = Math.floor(Math.random() * 1920) + 1
        item.x = fastDuckXCoordinate
      }
    })
